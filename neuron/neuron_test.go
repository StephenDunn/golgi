...
func TestSetupNeurons_ShouldAssignDownstreamToAllNeurons(t *testing.T) {
	neurons := NewNetwork(totalNeurons, WithThreshold(thresholdMin, thresholdMax), WithRefactory(refactory))
...
func TestNeuronTicker_RecievesTickSynapticThresholdUpdated(t *testing.T) {
	neuron := New(WithThreshold(thresholdMin, thresholdMax), WithRefactory(refactory))
	impulse := 40
